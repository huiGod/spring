# refresh.preInstantiateSingletons()实例化创建对象

在容器初始化流程中，refresh会实例化容器中所有bean对象，并进行初始化，由preInstantiateSingletons方法处理
...
比较重要的是 FactoryBean 类型的 bean，会默认对 beanName加上&前缀，然后进行其初始化。后续需要获取 FactoryBean 类本身对象时，也是需要加上&beanName，否则 beanName 获取的是 getObject 返回的对象

# getBean从容器中获取对象

通过AbstractBeanFactory工厂类从Spring容器中获取指定的bean对象，如果不存在则创建并缓存起来
...
protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
    ...
    try {
        // 在 bean 实例化创建前给BeanPostProcessors（InstantiationAwareBeanPostProcessor）后置处理器一个机会来直接返回对象
...
对象创建完成后，未设置属性前，会提前暴露到singletonFactories集合中
...
在通过singletonFactories获取提前暴露的对象时，可以通过后置处理器来修改对象，默认是直接返回
...
- @Autowired 注解：通过后置处理器AutowiredAnnotationBeanPostProcessor来通过 bean 类型完成值注入
...
如没有没有修改 BeanDefinition 的注入模式，而是给属性加上了@Autowired 注解，则是通过后置处理器AutowiredAnnotationBeanPostProcessor来完成自动注入

AutowiredAnnotationBeanPostProcessor 的属性注入是通过类型注入的，然后通过ByName进行过滤。如果是属性，则直接从Spring容器中根据类型获取bean，通过Field反射赋值；如果是方法，则获取方法的参数列表，从容器中获取对应类型的参数bean，获取到后通过Method反射调用方法
...
属性注入调用的是AutowiredFieldElement的inject方法。最终通过**反射设置Field字段**设置属性值
...
属性注入调用的是AutowiredMethodElement的inject方法。最终通过**反射调用Method方法**设置属性值
...
@Autowired属性注入，其实是根据ByType获取所有符合类型的Bean,然后通过ByName进行过滤
...
![Spring容器创建对象流程](2.实例化创建对象源码.assets/Spring容器创建对象流程.jpg)
