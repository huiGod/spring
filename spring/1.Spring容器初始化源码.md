...
因为nnotationConfigApplicationContext有父类，在执行构造方法时，会先执行父类构造方法：
...
解析bean的元数据后，加入一些默认值，构造成BeanDefinition再注册到容器中
...
2. 调用所有的BeanFactoryPostProcessor工厂后置处理器
...
## prepareBeanFactory()注册BeanPostProcessor

主要是往spring工厂中注册内置的bean后置处理器，包括很重要的ApplicationContextAwareProcessor，给spring工厂提供额外的一些能力，使得在后续实例化bean的时候能够回调做一些事情
...
调用所有的BeanFactoryPostProcessor类型，可以是自定义添加的，也有spring前面启动过程中内置加入的（这里最重要的是ConfigurationClassPostProcessor），并且执行有其先后顺序，核心处理流程：
