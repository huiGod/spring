...
# ConfigurationClassPostProcessor内置后置处理器

ConfigurationClassPostProcessor是默认的后置处理器，在容器启动时会注入进来，用来解析配置相关的 bean，非常重要，所以执行顺序的优先级非常高
...
在容器启动时，会先执行所有的 BeanDefinitionRegistryPostProcessor，也就是该对象的postProcessBeanDefinitionRegistry方法，主要负责解析配置。扫描容器中的所有 bean，处理需要解析的 bean，核心流程：

1. 扫描出带有@Configuration注解，并将其configurationClass属性设置为 full
2. 扫描出带有@Component、@ComponentScan、@Import、@ImportResource注解或者有@Bean方法则将configurationClass属性设置为lite
3. 遍历解析上述步骤遍历出的配置bean，比较关键的是解析@ComponentScan 和@import 注解
4. 将解析出来的类包装成 bean 并注册到容器中
